...
local function popup_win_opts(opts)
...
local function find_float_window(buffer_name)
  for _, winid in ipairs(vim.api.nvim_list_wins()) do
    local bufnr = vim.api.nvim_win_get_buf(winid)
    local name = vim.api.nvim_buf_get_name(bufnr)
    if name ~= '' and name:match(buffer_name .. '$') then
...
local function get_or_create_popup(opts)
  ...
  local winid, bufnr = find_float_window(buffer_name)
  if winid then
    vim.api.nvim_set_current_win(winid)
    vim.api.nvim_win_set_config(winid, popup_win_opts(opts))
    vim.api.nvim_set_option_value('winhighlight', 'Normal:NormalFloat,FloatBorder:FloatBorder', { win = winid })
    vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, {})
    apply_window_options(winid)
    ensure_close_key(bufnr)
    return bufnr
  ...
  local new_win = vim.api.nvim_open_win(bufnr, true, popup_win_opts(opts))
